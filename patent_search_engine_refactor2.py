...
class PatentSearchEngine:
    ...
    def load_data(self, data_path):
        ...
        patents = data[['patent_number', 'invention_title', 'abstract', 'descriptionText', 'assignee']]  # 添加专利号
        return patents

    def load_transformer_model(self):
        tokenizer = AutoTokenizer.from_pretrained("distilbert-base-uncased",force_download=False)
        model = AutoModel.from_pretrained("distilbert-base-uncased",force_download=False)
...
def display_results(question, results, search_engine):
    ...
    for index, result in results.iterrows():
        ...
        markdown_results.append(f"## 答案 {index+1}：\n\n{answer}\n")
        markdown_results.append(f"### 专利信息：\n\n{patent_info}\n")

        relevant_description = search_engine.extract_relevant_part(truncated_question, result['descriptionText'])
        markdown_results.append(f"### 相关描述片段：\n\n{relevant_description}\n")

    markdown_output = "\n".join(markdown_results)
    
    # 将结果写入 Markdown 文件
    with open("search_results.md", "a+", encoding="utf-8") as file:
        file.write(markdown_output)

    print("搜索结果已保存到 search_results.md 文件中")
